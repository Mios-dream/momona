...
| `/settings` | 个人资料、数据来源、手动内容和首页组件配置 | 仅本地开发 |
| `/404`      | 未找到页面                                 | 生产构建   |
...
| 手动内容                              | `manualItems` | 手动添加的资料库条目、社交链接和友联                          |
...
2. 在“数据来源”中启用来源，填写用户名或用户 ID，并勾选需要同步的内容。Steam 可直接填写公开个人页；完整游戏库需要填写 Steam Web API Key，凭据只保存在本地。SFACG 填写公开书架地址，例如 `https://p.sfacg.com/p/8933368/`。
3. 在“组件设置”和“内容管理”中调整首页组件、友联和手动条目。
