...
const artist = computed(
  () => state.value.currentSong?.artist || props.track?.artist || "选择曲目",
);
const cover = computed(
  () => state.value.currentSong?.cover || props.track?.cover || "",
);
...
.music-status-main strong,
.music-status-main small {
  overflow: hidden;
...
.music-status-play:hover {
  color: var(--purple-deep);
  background: #fff;
}
.music-status-pill :deep(.music-picker-trigger) {
  width: 20px;
  height: 28px;
  justify-content: center;
}
.music-status-pill :deep(.music-picker-trigger-copy) {
  display: none;
}
.music-status-pill :deep(.music-picker-trigger > svg) {
  color: var(--muted);
}
.music-status-pill :deep(.music-picker-panel) {
  top: calc(100% + 9px);
}
.is-playing .music-status-cover {
  box-shadow: 0 0 0 2px rgba(53, 165, 123, 0.24);
}
